                            ...
                            <div class="product_info_box">
                                ...
                                <div class="product_top_info">
                                    ...
                                    <div class="rate_box">
                                        ...
                                        <div class="product_sum" v-if="keywordChart && keywordChart.devicePc != null">

                                            <i class="ri-search-line"></i> <span>검색량</span> <strong>{{ (keywordChart.devicePc + keywordChart.deviceMobile).toLocaleString() }}개</strong>
        ...
        onMounted(async () => {
            ...
            try {
              ...
              //  alert(keywordCode);
                ...
                const respo = await api.get('http://localhost:8084/user/keyword/getKeywordHistoryChart',{
                    params: {
                        keywordCode: keywordCode,
                        limitCount: 30
                    }
                })
                historyChart.value = respo.data

                const dates = historyChart.value.map(item => item.date);
                const counts = historyChart.value.map(item => item.totalCnt);
                dates.reverse()
                
                const getScore = await api.get('http://localhost:8084/user/keyword/getScore',{
                params: {
                    score: keywordScore.value
                }
                })
                rankNm.value = getScore.data;

                const respon = await api.get('http://localhost:8084/user/keyword/getKeywordCategory',{
                    params: {
                        keywordCode: keywordCode
                    }
                })
                keywordCategory.value = respon.data
