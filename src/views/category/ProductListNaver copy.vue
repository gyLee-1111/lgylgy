                                    ...
                                    <div class="func">
                                        ...
                                        <div class="func_row">
                                            ...
                                            <div class="link">
                                                ...
                                                <!-- 상품 리뷰 모달 -->
                                                <div v-if="isModalOpen" class="modal modal_pop_product" @click.self="closeModal">
                                                <!-- 팝업 헤더 -->
                                                <div class="pop_header">
                                                    <div class="product_review">
                                                    <div class="thum">
                                                        <img :src="selectedProductImg" alt="상품 이미지" />
                                                    </div>
                                                    <div class="detail_info">
                                                        <strong class="title">{{ selectedProductNm }}</strong>
                                                        <div class="add_info">
                                                        <span class="review">
                                                            <i class="ri-edit-line"></i> 리뷰갯수 <strong>{{ selectedReviewCount }}</strong>개
                                                        </span>
                                                ...
                                                <div class="pop_body">
                                                    ...
                                                    <div class="func">
                                                        <button class="btn btn_md bg_gray">
                                                    ...
                                                    <div class="review_sum">
                                                        총 <strong>{{ filteredReviews.length }}</strong>개
                                                    </div>
                                                    <ul>
                                                        <li v-for="(review, index) in filteredReviews" :key="review.reviewCode">
                                                        <div class="cont_txt">{{ review.reviewInfo }}</div>
                                                        <div class="rate_result">
                                                            <i v-for="n in 5" :key="n" class="ri-star-fill" :class="{ not: n > review.score }"></i>
                                                            <span>{{ review.insertDt }}</span>
                                                        </div>
                                                        </li>
                                                    </ul>
                                                    </div>
...
export default {
  ...
  setup() {
    ...
    const selectedProductNm = ref('')
    const selectedProductImg = ref('')
    const selectedReviewCount = ref(0)
...
//            alert(keywordCode)
    ...
    const getTotalPriceNaver = async() =>{
        try{
            const res = await axios.post('http://localhost:8084/public/getTotalPriceNaver',null,
      {
        params: {
          keywordCode: keywordCode
        }
    ...
    const openReviewModal = async (productCode) => {
        const product = productList.value.find(p => p.productCode === productCode)
        if (product) {
            selectedProductNm.value = product.productNm
            selectedProductImg.value = 'http://localhost:8084/images/product/' + product.productImg
            selectedReviewCount.value = product.reviewCnt
        }
        try {
            const res = await axios.get('http://localhost:8084/reviewPop/getListReview', {
            params: { productCode }
            })
