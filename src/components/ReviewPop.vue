                ...
                <div class="detail_info">
                    ...
                    <div class="add_info" v-if="showMarketInfo">
                        ...
                        <span class="top_market">
                            Top5 시장규모 <strong>{{ formatKoreanCurrency(totalPrice) }}</strong>
                        </span>
        ...
        <div class="pop_body">
            ...
            <div class="review_top">
                <div class="rate_sort">
                    ...
                    <ul class="face_rating">
                        ...
                        <!--     <label class="title" :class="{ active: selectedRatings.includes(i) }">-->
                        <!--    <a href="#" class="title" :class="{ active: selectedRatings.length === 0 || selectedRatings.includes(i) }" @click.prevent="toggleRating(i)">-->
                            <a href="#" class="title" :class="{ active: selectedRatings.includes(i) }" @click.prevent="toggleRating(i)">
                                <input type="checkbox" class="rate_filter" :value="i" v-model="selectedRatings" style="display: none" />
                                <span class="face">
                                    <i :class="[
                                        'fa-regular',
            ...
            <div class="review_list">
                ...
                <div class="review_sum">
                    총 <strong>{{ filteredReviewList.length }}</strong>개
                </div>

                <ul>
                    <li v-for="review in filteredReviewList" :key="review.reviewCode">
                        <div class="cont_txt">
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const selectedRatings = ref([5,4,3,2,1])
        ...
        const filteredReviewList = computed(() => {
            ...
            return reviewList.value.filter(review =>
                selectedRatings.value.includes(review.reviewScore)
            )
        })
        const toggleRating = async (i) => {
            const index = selectedRatings.value.indexOf(i);

            if (index === -1) {
                // 없으면 추가
                selectedRatings.value.push(i);
            } else {
                // 있으면 제거
